...
void AES_CBC_encrypt_buffer(struct AES_ctx* ctx, uint8* buf, uint64 length);
void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, uint8* buf, uint64 length);
...
//        no IV should ever be reused with the same key
void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, uint8* buf, uint64 length);
